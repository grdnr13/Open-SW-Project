...
int main() {
	...
	for (int y = 0; y < input_gray.rows; y++)
		for (int x = 0; x < input_gray.cols; x++) {
			samples.at<float>(y * input_gray.cols + x, 0) = input_gray.at<G>(y, x);
			samples.at<float>(y * input_gray.cols + x, 1) = x / sigma_x;
			samples.at<float>(y * input_gray.cols + x, 2) = y / sigma_y;
		}
