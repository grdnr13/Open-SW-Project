...
int main() {
	...
	//Mat L = gaussianfilter(input_gray, 12, 7, 7, "zero-paddle"); 
	//Mat L = gaussianfilter(input_gray, 12, 7, 7, "mirroring");
	Mat L = gaussianfilter(input_gray, 12, 7, 7, "adjustkernel");
	
	//namedWindow("Blur with Low-pass filter", WINDOW_AUTOSIZE);
	//imshow("Blur with Low-pass filter", L);

	output = Unsharp(input_gray, L, k);
...
Mat Unsharp(const Mat original, const Mat low_pass, const float k) {
	...
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
			float value = (original.at<G>(i, j) - low_pass.at<G>(i, j)*k) / (1 - k);
			if (value < 0)value = 0;
