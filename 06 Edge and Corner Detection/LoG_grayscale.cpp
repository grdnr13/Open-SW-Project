		...
		for (int j = n; j < col + n; j++) {
			...
			for (int a = -n; a <= n; a++) {
				for (int b = -n; b <= n; b++) {
					...
					output.at<double>(i - n, j - n) += kernelvalue * (float)(input_mirror.at<double>(i + a, j + b));
				}
			}
		}
	}
...
Mat Laplacianfilter(const Mat input) {
	...
	for (int i = n; i < row + n; i++) {
		for (int j = n; j < col + n; j++) {
			...
			for (int a = -n; a <= n; a++) {
				for (int b = -n; b <= n; b++) {
					tmp += kernel.at<double>(a + n, b + n) * input_mirror.at<double>(i + a, j + b);
					output.at<double>(i - n, j - n) = (double)(visualize * sqrt(tmp * tmp));
				}
			}
		}
	}
