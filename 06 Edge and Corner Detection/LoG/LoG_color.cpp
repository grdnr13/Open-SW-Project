			...
			for (int a = -n; a <= n; a++) {
				for (int b = -n; b <= n; b++) {
					...
					for (int idx = 0; idx < 3; idx++)
						output.at<Vec3d>(i - n, j - n)[idx] += input_mirror.at<Vec3d>(i + a, j + b)[idx] * kernelvalue;
				}
			}
		}
	}
		...
		for (int j = n; j < col + n; j++) {
			...
			for (int a = -n; a <= n; a++) {
				for (int b = -n; b <= n; b++) {
					kernelvalue = kernel.at<double>(a + n, b + n);
					for (int idx = 0; idx < 3; idx++) {
						sum[idx] += kernel.at<double>(a + n, b + n) * input_mirror.at<Vec3d>(i + a, j + b)[idx];
						output.at<Vec3d>(i - n, j - n)[idx] = (double)(visualize * sqrt(sum[idx] * sum[idx]));
					}
				}
			}
		}
